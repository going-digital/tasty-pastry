...
static void init_shaders(STATE_T *state)
{
  ...
  const GLchar *fshader_source =
    ...
    "}"
    "float dodec(vec3 p) {"
      "vec4 a4 = vec4(0.000,0.526,0.851,-0.851);"
    ...
    "void main(){"
      ...
      "vec3 vrp=vec3(0,0,0);"
      "vec3 origin=vec3(sinf(t*.3*.7)*20.+vrp.x+20.,10.,sinf(t*.3*.6+.1591549431)*20.+vrp.z+14.);"
      "vec3 vpn=normalize(vrp-origin);"
      "vec3 u=normalize(cross(vuv,vpn));"
      "vec3 w=cross(vpn,u);"
      "vec3 scrCoord=origin+vpn+v.x*u*aa+v.y*w;"
      "vec3 direction=normalize(scrCoord-origin);"
      "const float maxd=200.;"
      "float s=0.1;"
      "float f=0.;"
      "s=dScene(origin+direction*f);"
      "for(int i=0;i<15;i++){"
      ...
      "vec3 n = normalize(vec3(s-dScene(p-e.xyy),s-dScene(p-e.yxy),s-dScene(p-e.yyx)));"
      "float z=dot(n,normalize(origin-p));"
      "if(f>maxd){"
